...
def predict():
    audio_blob=FileStorage(stream=open('uploads/recorded_audio.wav', "rb"))
    if audio_blob:
        predicted_genre, probabilities = predict_genre(audio_blob)
        
        plot_path = visualize_prediction(predicted_genre, probabilities)
        
        return render_template('home.html', prediction={"genre": predicted_genre, "plot_path": plot_path, 'probabilities': probabilities})
    else:
